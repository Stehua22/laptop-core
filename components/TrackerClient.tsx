...
export let CAD_TO_USD = 0.73; // fallback, updated by live fetch
    ...
    async function fetchRate() {
      try {
        ...
        if (rate && typeof rate === "number") {
          setCadToUsd(rate);
          CAD_TO_USD = rate; // update module-level export for other components
        }
      } catch {
        // silently fall back to 0.73
      }
    }
    fetchRate();
  }, []);
...
<<<<<<< HEAD
  ...
  const handleToggleRecommendation = async (category: string, laptopId: number) => {
    ...
    const exists = current.includes(laptopId);
    const newIds = exists
    ...
    try {
      await supabase
        ...
        .upsert({ category, laptop_ids: newIds }, { onConflict: "category" });
...
<<<<<<< HEAD
  const handleMoveToDeals = async (laptop: Laptop) => {
    ...
    try {
      ...
      showToast("❌ Failed to move to deals", "error");
    }
  };
...
<<<<<<< HEAD
      {showAdminPanel && (
        <AdminPanel
          ...
          onLaptopUpdated={handleLaptopUpdatedFromAdmin}
        />
      )}
      ...
      {showAuthModal && (
        ...
        >
          <div
            ...
            style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: "1.75rem", width: "100%", maxWidth: 400, margin: "1rem", boxShadow: "var(--shadow-lg)" }}
          >
            ...
            <p style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 20 }}>
              Enter the password to unlock admin actions.
            </p>
            <input
              type="password"
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 16 }}>
              <button onClick={() => setShowAuthModal(false)} style={{
                fontSize: 13, padding: "8px 18px", borderRadius: 9,
              ...
              <button onClick={submitAuth} style={{
                ...
                boxShadow: "0 4px 12px rgba(139,179,245,0.3)",
              }}>Unlock</button>
            </div>
          </div>
        </div>
