    ...
    <section>
      ...
      {laptops.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {laptops.map((l) => (
            <LaptopCard key={l.id} laptop={l} onSelect={onSelect} onHistory={onHistory} isAdmin={isAdmin} onMoveToDeals={onMoveToDeals} onDelete={onDelete} currency={currency} cadToUsd={cadToUsd} />
          ))}
        </div>
      )}
    </section>
