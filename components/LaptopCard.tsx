...
export default function LaptopCard({ laptop, onSelect, onHistory, isAdmin, onMoveToDeals, onDelete, currency = "CAD", cadToUsd = 0.73 }: Props) {
  ...
  return (
    <>
      <div className="laptop-card" onClick={() => window.location.href = `/laptop/${laptop.id}`}
        ...
        <div style={{ padding: "16px 20px 14px", flex: 1 }}>
          <div style={{ display: "inline-flex", alignItems: "center", background: "rgba(139,179,245,0.12)", color: "var(--accent)", borderRadius: 6, padding: "3px 8px", fontSize: 10, letterSpacing: "0.12em", marginBottom: 10, textTransform: "uppercase", fontWeight: 600 }}>{laptop.brand}</div>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
            <span style={{ fontWeight: 800, fontSize: "1.5rem", color: "var(--accent-3)", letterSpacing: "-0.02em", transition: "all 0.3s ease" }}>{fmt(price, currency, cadToUsd)}</span>
            {hasDiscount && (
              <>
        ...
        <div style={{ display: "flex", gap: 6, padding: "12px 16px 14px", borderTop: "1px solid var(--border)", flexWrap: "wrap" }} onClick={(e) => e.stopPropagation()}>
          ...
          {isAdmin && onMoveToDeals && (
            <button onClick={() => onMoveToDeals(laptop)}
              ...
              onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = "rgba(247,194,106,0.08)"; }}>🔥 Deal</button>
          )}
          {isAdmin && onDelete && (
