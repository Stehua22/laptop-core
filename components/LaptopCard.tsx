...
export default function LaptopCard({ laptop, onSelect, onHistory, isAdmin, onMoveToDeals, onDelete, currency = "CAD", cadToUsd = 0.73, compareSelected = false, onCompareToggle, compareDisabled = false }: Props) {
  ...
  return (
    <>
      <div
        ...
        style={{
          background: compareSelected ? "rgba(139,179,245,0.06)" : "var(--surface)",
          border: `1px solid ${compareSelected ? "var(--accent)" : hovered ? "rgba(139,179,245,0.35)" : "var(--border)"}`,
          borderRadius: 18,
          overflow: "hidden",
          display: "grid",
      ...
      >
        ...
        <div style={{
          ...
          {hasDiscount && (
            <div style={{ position: "absolute", top: 12, left: 12, background: "linear-gradient(135deg, #f7c26a, #f4a830)", color: "#1a1200", fontSize: 10, fontWeight: 900, borderRadius: 6, padding: "3px 8px", letterSpacing: "0.04em" }}>-{discountPct}%</div>
          )}
          {(laptop as any).is_deal && (
            <div style={{ position: "absolute", top: hasDiscount ? 38 : 12, left: 12, background: "rgba(106,247,184,0.15)", border: "1px solid rgba(106,247,184,0.3)", color: "var(--accent-3)", fontSize: 9, fontWeight: 800, borderRadius: 6, padding: "3px 8px" }}>HOT DEAL</div>
          )}
        ...
        <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", justifyContent: "space-between", minWidth: 0 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
              <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--accent)", background: "rgba(139,179,245,0.12)", border: "1px solid rgba(139,179,245,0.25)", borderRadius: 6, padding: "3px 9px" }}>{laptop.brand}</span>
        ...
        {/* PRICE + ACTIONS */}
        <div style={{ borderLeft: "1px solid var(--border)", padding: "20px 22px", display: "flex", flexDirection: "column", alignItems: "flex-end", justifyContent: "space-between", minWidth: 190, background: hovered ? "linear-gradient(180deg, rgba(139,179,245,0.04) 0%, transparent 100%)" : "transparent", transition: "background 0.2s" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 7, width: "100%" }} onClick={e => e.stopPropagation()}>
            <button
              onClick={() => window.location.href = `/laptop/${laptop.id}`}
              style={{ width: "100%", border: "none", borderRadius: 10, background: "linear-gradient(135deg, var(--accent), #6ab4f5)", color: "#fff", fontSize: 12, fontWeight: 700, padding: "10px 14px", cursor: "pointer", transition: "opacity 0.15s, transform 0.15s", boxShadow: "0 4px 14px rgba(139,179,245,0.25)" }}
              onMouseEnter={e => { e.currentTarget.style.opacity = "0.88"; e.currentTarget.style.transform = "scale(1.02)"; }}
              onMouseLeave={e => { e.currentTarget.style.opacity = "1"; e.currentTarget.style.transform = "scale(1)"; }}
            >View Details →</button>
            <div style={{ display: "flex", gap: 6 }}>
              <button onClick={() => onHistory(laptop)} style={{ flex: 1, background: "transparent", border: "1px solid var(--border)", borderRadius: 8, color: "var(--text-muted)", fontSize: 10, fontWeight: 600, padding: "7px 0", cursor: "pointer", transition: "all 0.15s" }}
              ...
              >📈 History</button>
              <button onClick={() => { setPendingUrl(visitUrl); setShowWarning(true); }} style={{ flex: 1, background: "transparent", border: "1px solid var(--border)", borderRadius: 8, color: "var(--text-muted)", fontSize: 10, fontWeight: 600, padding: "7px 0", cursor: "pointer", transition: "all 0.15s" }}
            ...
            {isAdmin && (
              <div style={{ display: "flex", gap: 6 }}>
                {onMoveToDeals && <button onClick={() => onMoveToDeals(laptop)} style={{ flex: 1, background: "rgba(247,194,106,0.08)", border: "1px solid rgba(247,194,106,0.3)", borderRadius: 7, color: "var(--accent-2)", fontSize: 10, padding: "6px 0", cursor: "pointer" }}
                ...
                >🔥 Deal</button>}
                {onDelete && <button onClick={() => onDelete(laptop.id)} style={{ flex: 1, background: "rgba(247,106,106,0.08)", border: "1px solid rgba(247,106,106,0.3)", borderRadius: 7, color: "var(--accent-red)", fontSize: 10, padding: "6px 0", cursor: "pointer" }}
