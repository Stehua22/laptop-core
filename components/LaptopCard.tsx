...
export default function LaptopCard({ laptop, onSelect, onHistory, isAdmin, onMoveToDeals, onDelete }: Props) {
    ...
    <>
      ...
      >
        ...
        <div
          ...
          style={{ height: 3, background: `linear-gradient(90deg, var(--accent), var(--accent-3))`, opacity: 0.8 }}
        />
        ...
        {(laptop as any).image_url && !imgError ? (
          <div style={{
            ...
            <img
              ...
              style={{
                maxHeight: 128, maxWidth: "100%", objectFit: "contain",
        ...
        <div style={{ padding: "16px 20px 14px", flex: 1 }}>
          <div style={{
            ...
            background: "rgba(139,179,245,0.12)",
            color: "var(--accent)",
          ...
          <div style={{ fontWeight: 700, fontSize: "1rem", color: "var(--text)", marginBottom: 3, lineHeight: 1.3 }}>
            {laptop.model}
          </div>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
            <span style={{ fontWeight: 800, fontSize: "1.5rem", color: "var(--accent-3)", letterSpacing: "-0.02em" }}>
              {fmt(price)}
            </span>
            {hasDiscount && (
              <>
                ...
                <span style={{
                  background: "rgba(247,194,106,0.15)", color: "var(--accent-2)",
        ...
        <div
          style={{
            display: "flex", gap: 6, padding: "12px 16px 14px",
        ...
        >
          {[
            ...
            },
            {
              label: "🔗 Visit",
              onClick: () => {
                const url = laptop.url || `https://www.google.com/search?q=${encodeURIComponent(laptop.brand + " " + laptop.model)}`;
                setPendingUrl(url);
                setShowWarning(true);
              },
              hoverBorder: "var(--accent-3)",
              hoverColor: "var(--accent-3)",
            },
          ...
          {isAdmin && onMoveToDeals && (
            <button
              ...
              style={{
                flex: 1, background: "rgba(247,194,106,0.08)",
                border: "1px solid rgba(247,194,106,0.4)",
                borderRadius: 8, color: "var(--accent-2)",
          ...
          {isAdmin && onDelete && (
            <button
              ...
              style={{
                flex: 1, background: "rgba(247,106,106,0.08)",
      ...
      {showWarning && (
        <div
          ...
          style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(4px)" }}
          onClick={() => setShowWarning(false)}
        >
          <div
            className="modal-content"
            style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: "1.5rem", maxWidth: 360, margin: "1rem", boxShadow: "var(--shadow-lg)" }}
            onClick={(e) => e.stopPropagation()}
          >
            <p style={{ fontWeight: 700, fontSize: 15, marginBottom: 8 }}>⚠️ Price Warning</p>
            <p style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 20, lineHeight: 1.6 }}>
              Prices may not be accurate. The price shown may differ from what's currently on the store website.
            </p>
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              <button onClick={() => setShowWarning(false)} style={{ fontSize: 13, padding: "8px 18px", borderRadius: 8, border: "1px solid var(--border)", background: "transparent", color: "inherit", cursor: "pointer" }}>
                Cancel
              </button>
              <button onClick={() => { window.open(pendingUrl, "_blank"); setShowWarning(false); }} style={{ fontSize: 13, padding: "8px 18px", borderRadius: 8, border: "none", background: "var(--accent)", color: "#fff", cursor: "pointer", fontWeight: 600 }}>
                Continue →
              </button>
            </div>
          </div>
