...
import AdminPriceHistoryModal from "@/components/AdminPriceHistoryModal";
...
export default function AdminPage() {
  ...
  const [tab, setTab] = useState<"laptops" | "add" | "picks" | "history">("laptops");
  const [editingHistoryLaptop, setEditingHistoryLaptop] = useState<Laptop | null>(null);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 20px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32, flexWrap: "wrap", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {(["laptops", "picks", "history", "add"] as const).map(t => (
              <button key={t} onClick={() => setTab(t)} style={{ fontSize: 13, padding: "8px 18px", borderRadius: 99, border: `1px solid ${tab === t ? "var(--accent)" : "var(--border)"}`, background: tab === t ? "var(--accent)" : "transparent", color: tab === t ? "#fff" : "var(--text-muted)", cursor: "pointer", fontWeight: 600 }}>
                {t === "laptops" ? "📋 All Laptops" : t === "picks" ? "🎓 Best Picks" : t === "history" ? "📈 Price History" : "+ Add New"}
              </button>
            ))}
          </div>
        </div>
        ...
        {tab === "history" && (
          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: 24 }}>
            <h2 style={{ fontWeight: 700, fontSize: 16, marginBottom: 4 }}>Price History Editor</h2>
            <p style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 20 }}>Select a laptop to view, edit, or clear its price history.</p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 10, marginBottom: 16 }}>
              <input placeholder="🔍 Search brand or model..." value={historySearch} onChange={(e) => setHistorySearch(e.target.value)} style={{ ...inputStyle, padding: "10px 14px", fontSize: 13 }} />
              <select value={historyBrandFilter} onChange={(e) => setHistoryBrandFilter(e.target.value)} style={{ ...inputStyle, width: "auto", minWidth: 140, padding: "10px 14px", fontSize: 13, cursor: "pointer" }}>
                <option value="">All Brands</option>
                {brands.map(b => <option key={b} value={b}>{b}</option>)}
              </select>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 80px 100px 90px", gap: 8, paddingBottom: 8, borderBottom: "1px solid var(--border)", marginBottom: 8 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4, maxHeight: 520, overflowY: "auto" }}>
              {laptops
                .filter(l => {
                  ...
                  const matchBrand = !historyBrandFilter || l.brand === historyBrandFilter;
                  return matchSearch && matchBrand;
                })
                .map((laptop) => (
                  <div key={laptop.id}
                  ...
                  >
                    ...
                    <span style={{ fontSize: 13, fontWeight: 700, color: "var(--accent)", textAlign: "right" }}>{fmt(laptop.current_price ?? laptop.retail_price ?? 0)}</span>
                    <div style={{ display: "flex", justifyContent: "flex-end" }}>
                      <button onClick={() => setEditingHistoryLaptop(laptop)}
      ...
      {editingHistoryLaptop && (
        <AdminPriceHistoryModal
          laptop={editingHistoryLaptop}
          currency="CAD"
          onClose={() => setEditingHistoryLaptop(null)}
          onUpdated={(updated) => {
            setEditingHistoryLaptop(updated);
